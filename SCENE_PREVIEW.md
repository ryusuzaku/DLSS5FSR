...
next capture runs. A measured pass took about 3.5 minutes on the development
...
pass, the sidecar atomically replaces `candidate_preview.bin`; the shim
...
preview hash and timings. Stop with Ctrl+C; `--max-updates 1` processes one
scene and exits. A stopped game causes the capture wait to time out after
120 seconds, leaving the last valid preview available.
