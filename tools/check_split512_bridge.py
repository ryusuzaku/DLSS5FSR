...
def run_case(width=16,height=8,source_path=None,output_root=None):
    ...
    if width not in (8,16,32) or height!=8:raise ValueError('covered extents are 8x8, 16x8 and 32x8')
    block=ROOT/'build/split512_spatial'/f'block30-{width}x{height}-s2'
    source=Path(source_path) if source_path is not None else block/'final_raw_device.f32'
    ...
    folder=Path(output_root) if output_root is not None else ROOT/'build'/('split512_bridge' if width==16 else f'split512_bridge_{width}x{height}')
