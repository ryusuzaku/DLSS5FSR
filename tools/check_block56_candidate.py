...
def run(case='seeded',first_window=False,block=56,width=WIDTH,height=HEIGHT):
    if case not in ('seeded','zero','image_fp8','image_from_block48','image_from39','image_from38','image_encoder_skip30','image_candidate_vit16'):raise ValueError('unknown skip case')
    ...
    source=((FROM39/f'upsample56_{case.removeprefix("image_")}'/'merged_device.f32' if block==56 else
            ...
            if case in ('image_from39','image_from38','image_encoder_skip30','image_candidate_vit16') else
    ...
    root=(FROM39/f'peer_decoder56_{case.removeprefix("image_")}'/f'block{block}' if case in ('image_from39','image_from38','image_encoder_skip30','image_candidate_vit16') else
    ...
    report=dict(block=block,case=case,shift=shift,extent=[width,height,CHANNELS],
                ...
                encoder14_skip=('public same-image FP8-boundary control' if case in ('image_fp8','image_from_block48','image_from39','image_from38','image_encoder_skip30','image_candidate_vit16') else 'synthetic control'),
...
if __name__=='__main__':
    ...
    p.add_argument('--case',choices=('seeded','zero','image_fp8','image_from_block48','image_from39','image_from38','image_encoder_skip30','image_candidate_vit16'),default='seeded')
