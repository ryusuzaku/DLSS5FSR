...
def run(case):
    if case not in ('image_half','image_fp8','from62_fp8','from56_fp8'):raise ValueError('bad case')
    ...
    if case in ('from62_fp8','from56_fp8'):speer=F(speer)
    ...
    if case in ('from62_fp8','from56_fp8'):
        upstream_case='image_fp8' if case=='from62_fp8' else 'from56_fp8'
        audit_path=ROOT/'build'/('peer_decoder62_tail_audit' if case=='from62_fp8' else
                                 'peer_decoder62_tail_audit_from56')/'manifest.json'
        audit=json.loads(audit_path.read_text())
        input_path=ROOT/'build/decoder65_candidate_derived'/upstream_case/'output/output_device.f32'
    ...
    out=ROOT/'build/upsample66_prefix_derived'/case
...
if __name__=='__main__':
    ...
    p.add_argument('--case',choices=('image_half','image_fp8','from62_fp8','from56_fp8'),default='image_half')
