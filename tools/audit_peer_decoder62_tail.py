...
def run(case='image_fp8'):
    ...
    prefix=(FROM39/f'upsample62_{case.removesuffix("_fp8")}' if case in ('from39_fp8','from38_fp8') else
    ...
    for block in range(62,66):
        root=(FROM39/f'peer_decoder62_{case.removesuffix("_fp8")}'/f'block{block}' if case in ('from39_fp8','from38_fp8') else
    ...
    out=ROOT/'build'/(f'peer_decoder62_tail_audit_{case.removesuffix("_fp8")}' if case in ('from39_fp8','from38_fp8') else
...
if __name__=='__main__':
    ...
    p.add_argument('--case',choices=('image_half','image_fp8','from56_fp8','from48_fp8','from39_fp8','from38_fp8'),default='image_fp8')
