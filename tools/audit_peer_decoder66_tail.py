...
def run(cases=None):
    ...
    for case in (cases or ('image_half','image_fp8','from62_fp8','from56_fp8')):
        ...
        prefix=(FROM39/f'upsample66_{suffix}' if case in ('from39_fp8','from38_fp8','from_encoder_skip30_fp8','from_candidate_vit16_fp8','from_candidate_encoder22_fp8','from_candidate_encoder14_fp8','from_candidate_encoder8_fp8') else
        ...
        if case in ('from62_fp8','from56_fp8','from48_fp8','from39_fp8','from38_fp8','from_encoder_skip30_fp8','from_candidate_vit16_fp8','from_candidate_encoder22_fp8','from_candidate_encoder14_fp8','from_candidate_encoder8_fp8'):
            audit_dir=('peer_decoder62_tail_audit' if case=='from62_fp8' else
                       ...
                       f'peer_decoder62_tail_audit_{suffix}')
        ...
        for block in range(66,70):
            root=(FROM39/f'peer_decoder66_{suffix}'/f'block{block}' if case in ('from39_fp8','from38_fp8','from_encoder_skip30_fp8','from_candidate_vit16_fp8','from_candidate_encoder22_fp8','from_candidate_encoder14_fp8','from_candidate_encoder8_fp8') else
    ...
    out=(ROOT/'build'/f'peer_decoder66_tail_audit_{"encoder_skip30" if cases==["from_encoder_skip30_fp8"] else "candidate_vit16" if cases==["from_candidate_vit16_fp8"] else "candidate_encoder22" if cases==["from_candidate_encoder22_fp8"] else "candidate_encoder14" if cases==["from_candidate_encoder14_fp8"] else "candidate_encoder8" if cases==["from_candidate_encoder8_fp8"] else cases[0].removesuffix("_fp8")}' if cases in (['from39_fp8'],['from38_fp8'],['from_encoder_skip30_fp8'],['from_candidate_vit16_fp8'],['from_candidate_encoder22_fp8'],['from_candidate_encoder14_fp8'],['from_candidate_encoder8_fp8']) else
...
if __name__=='__main__':
    ...
    parser.add_argument('--case',choices=('from48_fp8','from39_fp8','from38_fp8','from_encoder_skip30_fp8','from_candidate_vit16_fp8','from_candidate_encoder22_fp8','from_candidate_encoder14_fp8','from_candidate_encoder8_fp8'))
