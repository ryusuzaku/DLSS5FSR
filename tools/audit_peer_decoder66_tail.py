...
def run(cases=None):
    ...
    for case in (cases or ('image_half','image_fp8','from62_fp8','from56_fp8')):
        ...
        prefix=(FROM39/f'upsample66_{suffix}' if case in ('from39_fp8','from38_fp8','from_encoder_skip30_fp8') else
        ...
        for block in range(66,70):
            root=(FROM39/f'peer_decoder66_{suffix}'/f'block{block}' if case in ('from39_fp8','from38_fp8','from_encoder_skip30_fp8') else
    ...
    out=(ROOT/'build'/f'peer_decoder66_tail_audit_{"encoder_skip30" if cases==["from_encoder_skip30_fp8"] else cases[0].removesuffix("_fp8")}' if cases in (['from39_fp8'],['from38_fp8'],['from_encoder_skip30_fp8']) else
...
if __name__=='__main__':
    ...
    parser.add_argument('--case',choices=('from48_fp8','from39_fp8','from38_fp8','from_encoder_skip30_fp8'))
