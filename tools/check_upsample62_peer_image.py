...
def run(case):
    if case not in ('image_half','image_fp8','from56_fp8','from48_fp8','from39_fp8','from38_fp8'):raise ValueError('bad case')
    ...
    if case in ('from56_fp8','from48_fp8','from39_fp8','from38_fp8'):speer=F(speer)
    p128=peer_to_native_multihead(np.arange(128))
    p64=peer_to_native_multihead(np.arange(64))
    x=np.empty_like(xpeer);x[...,p128]=xpeer
    upstream=None
    if case in ('from56_fp8','from48_fp8','from39_fp8','from38_fp8'):
        ...
        input_path=(FROM39/f'peer_decoder56_{case.removesuffix("_fp8")}'/'block61/output/output_device.f32'
        ...
        upstream=dict(audit_sha256=digest(audit_path),block61_native_sha256=digest(input_path),
                      boundary=('AMD candidate block39-61 from public ViT38/skip30 and same-image skips'
    ...
    out=(FROM39/f'upsample62_{case.removesuffix("_fp8")}' if case in ('from39_fp8','from38_fp8') else
...
if __name__=='__main__':
    ...
    p.add_argument('--case',choices=('image_half','image_fp8','from56_fp8','from48_fp8','from39_fp8','from38_fp8'),default='image_half')
