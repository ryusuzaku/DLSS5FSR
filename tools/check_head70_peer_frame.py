...
def run(size=256,chunk_windows=32,latent_case='public',output_root=None,
    ...
    if latent_case=='public':
        ...
        latent_path=(Path.home()/'DLSS5FSR-build-offload'/f'peer_decoder66_{"encoder_skip30" if latent_case=="from_encoder_skip30_fp8" else "candidate_vit16" if latent_case=="from_candidate_vit16_fp8" else "candidate_encoder22" if latent_case=="from_candidate_encoder22_fp8" else "candidate_encoder14" if latent_case=="from_candidate_encoder14_fp8" else "candidate_encoder8" if latent_case=="from_candidate_encoder8_fp8" else latent_case.removesuffix("_fp8")}'/'block69/output/output_device.f32'
                     if latent_case in ('from39_fp8','from38_fp8','from_encoder_skip30_fp8','from_candidate_vit16_fp8','from_candidate_encoder22_fp8','from_candidate_encoder14_fp8','from_candidate_encoder8_fp8') else
        ...
        first_block=39 if latent_case in ('from38_fp8','from_encoder_skip30_fp8','from_candidate_vit16_fp8','from_candidate_encoder22_fp8','from_candidate_encoder14_fp8','from_candidate_encoder8_fp8') else 40 if latent_case=='from39_fp8' else 48 if latent_case=='from48_fp8' else 56 if latent_case=='from56_fp8' else 62 if latent_case=='from62_fp8' else 66
        latent_origin=(f'RX9070XT candidate encoder5-30, ViT31-38, and decoder{first_block}-69; encoder14/8 skips candidate, encoder4 skip public; 16-token attention experimental'
                       if latent_case=='from_candidate_encoder8_fp8' else
                       f'RX9070XT candidate encoder9-30, ViT31-38, and decoder{first_block}-69; encoder14 skip candidate, encoder8/4 skips public; 16-token attention experimental'
    ...
    report=dict(size=[size,size],windows=len(windows),chunk_windows=chunk_windows,
                ...
                native_amd_encoder15_22_candidate_executed=latent_case in ('from_candidate_encoder22_fp8','from_candidate_encoder14_fp8','from_candidate_encoder8_fp8'),
                native_amd_vit31_38_candidate_executed=latent_case in ('from_candidate_vit16_fp8','from_candidate_encoder22_fp8','from_candidate_encoder14_fp8','from_candidate_encoder8_fp8'),
                native_amd_decoder66_69_executed=latent_case!='public',
                native_amd_decoder62_69_executed=latent_case in ('from62_fp8','from56_fp8','from48_fp8','from39_fp8','from38_fp8','from_encoder_skip30_fp8','from_candidate_vit16_fp8','from_candidate_encoder22_fp8','from_candidate_encoder14_fp8','from_candidate_encoder8_fp8'),
                native_amd_decoder56_69_executed=latent_case in ('from56_fp8','from48_fp8','from39_fp8','from38_fp8','from_encoder_skip30_fp8','from_candidate_vit16_fp8','from_candidate_encoder22_fp8','from_candidate_encoder14_fp8','from_candidate_encoder8_fp8'),
                native_amd_decoder48_69_executed=latent_case in ('from48_fp8','from39_fp8','from38_fp8','from_encoder_skip30_fp8','from_candidate_vit16_fp8','from_candidate_encoder22_fp8','from_candidate_encoder14_fp8','from_candidate_encoder8_fp8'),
                native_amd_decoder40_69_executed=latent_case in ('from39_fp8','from38_fp8','from_encoder_skip30_fp8','from_candidate_vit16_fp8','from_candidate_encoder22_fp8','from_candidate_encoder14_fp8','from_candidate_encoder8_fp8'),
                native_amd_decoder39_69_executed=latent_case in ('from38_fp8','from_encoder_skip30_fp8','from_candidate_vit16_fp8','from_candidate_encoder22_fp8','from_candidate_encoder14_fp8','from_candidate_encoder8_fp8'),
