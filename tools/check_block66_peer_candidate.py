...
def run(block=66,case='seeded',first_window=False,width=WIDTH,height=HEIGHT):
    ...
    if case not in ('seeded','zero','image_half','image_fp8','from62_fp8','from56_fp8','from48_fp8','from39_fp8','from38_fp8','from_encoder_skip30_fp8'):
    ...
    root=(offload/f'peer_decoder66_{suffix}'/f'block{block}' if case in ('from39_fp8','from38_fp8','from_encoder_skip30_fp8') else
    ...
    report=dict(block=block,case=case,shift=shift,extent=[width,height,CHANNELS],
                ...
                encoder_skips=('public same-image block4 skip' if case.startswith('image_') or case in ('from62_fp8','from56_fp8','from48_fp8','from39_fp8','from38_fp8','from_encoder_skip30_fp8') else
...
if __name__=='__main__':
    ...
    p.add_argument('--case',choices=('seeded','zero','image_half','image_fp8','from62_fp8','from56_fp8','from48_fp8','from39_fp8','from38_fp8','from_encoder_skip30_fp8'),default='seeded')
