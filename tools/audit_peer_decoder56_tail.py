...
def run(case='image_fp8'):
    if case not in ('image_fp8','image_from_block48','image_from39','image_from38','image_encoder_skip30','image_candidate_vit16','image_candidate_encoder22','image_candidate_encoder14','image_candidate_encoder8'):raise ValueError('bad case')
    source=json.loads((SOURCE/'manifest.json').read_text())
    order=peer_to_native_multihead(np.arange(128))
    prefix=(FROM39/f'upsample56_{case.removeprefix("image_")}' if case in ('image_from39','image_from38','image_encoder_skip30','image_candidate_vit16','image_candidate_encoder22','image_candidate_encoder14','image_candidate_encoder8') else
    ...
    if not p['hip_projection_merge_exact'] or \
       ...
       p['source_model_sha256']!=source['model_sha256'] or \
       p.get('source_image_sha256',source['image_sha256'])!=source['image_sha256']:
        raise ValueError('prefix provenance/exactness differs')
    previous=prefix/'merged_device.f32'
    stages={}
    for block in range(56,62):
        root=(FROM39/f'peer_decoder56_{case.removeprefix("image_")}'/f'block{block}' if case in ('image_from39','image_from38','image_encoder_skip30','image_candidate_vit16','image_candidate_encoder22','image_candidate_encoder14','image_candidate_encoder8') else
    ...
    out=(ROOT/'build'/f'peer_decoder56_tail_audit_{case.removeprefix("image_")}' if case in ('image_from39','image_from38','image_encoder_skip30','image_candidate_vit16','image_candidate_encoder22','image_candidate_encoder14','image_candidate_encoder8') else
...
if __name__=='__main__':
    ...
    parser.add_argument('--case',choices=('image_fp8','image_from_block48','image_from39','image_from38','image_encoder_skip30','image_candidate_vit16','image_candidate_encoder22','image_candidate_encoder14','image_candidate_encoder8'),default='image_fp8')
